/**
 * key-index.mjs
 * Game UI Studio v0.3 — K-C 키 인덱스 재구성기 (MVP 비활성 스텁)
 * ESM, Node>=18, 외부 의존성 0.
 *
 * MVP에서 비활성: K-B(ManagedMarker 마커)가 진실원천.
 ...
 *   ManagedMarker.stableKey → GameObject 참조 매핑을 구성한다.
 *   현재는 K-B(ManagedMarker 마커 컴포넌트)가 진실원천이므로
 *   이 함수는 사용되지 않는다.
 *
