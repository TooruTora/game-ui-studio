---
description: Game UI Studio 매니페스트 강제 규약 단일 정본(SSOT). ui-designer 및 파이프라인 전 구성원이 따르는 유일한 강제 규약 문서입니다. $ARGUMENTS로 특정 규약 항목을 조회할 수 있습니다.
---

# Game UI Studio 매니페스트 강제 규약 (SSOT)
...
매니페스트 element의 `key` 필드는 **Unity 프리팹 내 ManagedMarker 컴포넌트의 `stableKey`** 에 그대로 박히는 영속 식별자입니다. 한 번 정하면 변경하지 않습니다.
