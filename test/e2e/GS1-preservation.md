...
> **Unity 환경 필요**: 이 e2e 시나리오는 Unity Editor 및 ManagedMarker 컴포넌트가 설치된 환경에서만 실행 가능하다.  
...
Game UI Studio가 생성한 프리팹에 부착된 `ManagedMarker.stableKey`가 다음 작업 후에도 보존됨을 보장한다:
...
```
Action: Game UI Studio → Assemble RosterScreen
Expected:
  - Assets/UI/Scenes/RosterScreen.prefab 생성
  - 각 element의 GameObject에 ManagedMarker 컴포넌트 부착
  - ManagedMarker.stableKey === element.key (계약 §(i))
```

수용 기준 (Unity Inspector에서 확인):

| GameObject | ManagedMarker.stableKey |
|------------|---------------------|
...
Expected:
  ...
  - ManagedMarker.stableKey: "btn.recruit" 유지됨
  - 프리팹 저장 성공
```

수용 기준:
- ManagedMarker.stableKey가 수동 부착 후에도 "btn.recruit"로 유지됨
- 다른 ManagedMarker 컴포넌트 미영향
...
```
Action: Game UI Studio → Assemble RosterScreen (수정된 manifest로)
Expected:
  - Btn_Recruit의 라벨이 "모집"으로 업데이트
  - MercRecruitView.cs 컴포넌트 보존 (수동 부착 유지)
  - 모든 ManagedMarker.stableKey 불변
```
...
| Btn_Recruit.ManagedMarker.stableKey | "btn.recruit" | __ |
| Btn_Recruit에 MercRecruitView 존재 | true | __ |
| panel.roster.ManagedMarker.stableKey | "panel.roster" | __ |
| list.mercs.ManagedMarker.stableKey | "list.mercs" | __ |
...
1. 재조립 후 어떤 GameObject의 `ManagedMarker.stableKey`가 변경됨
