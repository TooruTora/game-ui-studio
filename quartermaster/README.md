...
# 플러그인 설치
/plugin install quartermaster@quartermaster-marketplace
```

### 2. 로컬 설치 (개발용)

```bash
/plugin install ./quartermaster
```
...
```bash
# 자연어로 UI 화면 설계 시작
/qm 용병 목록 화면을 만들어줘. 상단에 타이틀, 스크롤 가능한 용병 카드 리스트, 하단에 고용 버튼이 필요해.

# 강제 규약 조회
/quartermaster:manifest-schema

# 정규화 규칙 조회
/quartermaster:prefab-normalize

# 직접 오케스트레이션 스킬 호출
/quartermaster:quartermaster 기존 RosterScreen 프리팹에 필터 패널 추가해줘
```
...
- **안정키(key) 영속** — `QmManaged.stableKey`와 1:1 매핑, 변경 불가
