/**
 * env-check.mjs
 * Quartermaster v0.3 — 환경 사전점검 (SessionStart hook용)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!result.node.ok) {
    process.stderr.write(
      `[quartermaster] Node.js >= 18 required, found ${result.node.version}. Upgrade Node.js to continue.\n`
    );
    process.exit(1);
  }
  ...
  if (!result.unity.ok || !result.mcp.ok) {
    process.stdout.write(
      '[quartermaster] Unity/MCP 미가용 — 트랙 A(매니페스트/프리뷰)는 계속 진행 가능합니다.\n\n'
    );
  }
