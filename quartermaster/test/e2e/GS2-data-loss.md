# GS2 — 데이터 무손실 골든 시나리오 (D0.8 QmManaged Sync)

> **Unity 환경 필요**: 이 e2e 시나리오는 Unity Editor 및 실제 `Assets/Quartermaster/` 디렉토리가 필요하다.  
> **현재 상태: RED (실패우선)** — D0.8 QmManaged 동기화가 아직 미구현이므로 이 시나리오는 현재 실패한다.  
...
사용자가 `Assets/Quartermaster/` 내 파일을 직접 수정한 후 Quartermaster 플러그인을 재설치하거나 버전 업그레이드해도 수정 내용이 무손실임을 보장한다.
...
| 환경 | 데이터 경로 | 비고 |
|------|------------|------|
| Claude Code 플러그인 | `${CLAUDE_PLUGIN_DATA}/quartermaster/` | Claude 플러그인 데이터 영역 |
| Unity 프리팹 | `Assets/Quartermaster/` | Unity가 `Assets/` 아래만 인식 (제약) |
| 공유 카탈로그 | `${CLAUDE_PLUGIN_DATA}/quartermaster/catalog.json` | 양쪽이 참조 |

> **Unity 제약 명문화**: Unity Editor는 `Assets/` 하위 파일만 AssetDatabase로 인식한다. 따라서 Quartermaster가 생성하는 프리팹 파일은 반드시 `Assets/Quartermaster/` 또는 `Assets/UI/Prefabs/`에 위치해야 한다. `${CLAUDE_PLUGIN_DATA}`에 프리팹을 두면 Unity가 임포트하지 못한다.

---

## 콘텐츠 해시 + 3-way 결정표 (4상한)

사용자 수정 감지: 파일의 SHA-256 해시를 `.quartermaster-hashes.json`에 기록.
...
```
전제: Quartermaster v0.2 설치, Assets/Quartermaster/catalog.json 미수정
Action: Quartermaster v0.3으로 업그레이드
Expected:
  - catalog.json이 v0.3 내용으로 자동 갱신
  - .quartermaster-hashes.json 해시 레코드 갱신
...
Action: Quartermaster v0.3으로 업그레이드 (v0.3이 다른 컴포넌트만 변경)
Expected:
...
```
전제: Assets/Quartermaster/RosterScreen.prefab 존재, QmManaged.stableKey 부착됨
Action: Quartermaster 언인스톨 → 재인스톨
Expected:
  - RosterScreen.prefab 무손실 (Unity AssetDatabase가 관리하는 Assets/ 영역)
  - .quartermaster-hashes.json 복원 또는 재초기화
  - 재조립 시 기존 stableKey 인식하여 add 대신 override 발생
수용기준:
  - planMerge 재실행 시 ops에 add 없고 override만 (또는 ops 비어있음)
  - QmManaged.stableKey 전체 보존
```
...
`.quartermaster-hashes.json` (예시):
```json
{
  "version": "0.3.0",
  "files": {
    "Assets/Quartermaster/catalog.json": {
